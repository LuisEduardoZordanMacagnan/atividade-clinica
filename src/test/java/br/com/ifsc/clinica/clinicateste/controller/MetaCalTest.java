...
public class MetaCalTest {
    ...
    private double extractMeta(long id, int nivel, boolean isEmagrecer) throws Exception {
        Map<String, Object> map = (Map<String, Object>) pc
                .getMetaCal(id, nivel, isEmagrecer)
                .getBody();

        return (double) map.get("metaCal");
    }

    @Test
    void testMetaCal() throws Exception {

        assertEquals(1947.50, extractMeta(1, 2, true), 0.01);
        assertEquals(2385.14, extractMeta(2, 3, false), 0.01);
        assertEquals(3107.63, extractMeta(3, 5, true), 0.01);
        assertEquals(1798.80, extractMeta(4, 1, false), 0.01);
    }
}
